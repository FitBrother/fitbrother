...
- `0060_complete_onboarding_v2.sql` — **correção durante a implementação:** a função real não se chama mais `complete_onboarding` desde a migration `0054` (M6) — ela foi renomeada para `complete_onboarding_impl` e envolvida por um wrapper `SECURITY DEFINER` (`complete_onboarding`) que valida consentimento obrigatório (`fitbrother_assert_required_consents`) antes de delegar. A `0060` reescreve **`complete_onboarding_impl`**, não o wrapper — do contrário o `CREATE OR REPLACE FUNCTION public.complete_onboarding(payload jsonb)` teria substituído silenciosamente o wrapper de consentimento do M6. Remove o cálculo de `v_kcal_factor`/`v_protein_per_kg`/etc.; passa a ler `payload->'targets'` (objeto já computado: `bmr_kcal, tdee_kcal, tdee_source, kcal, protein_g, carbs_g, fat_g, fiber_g, warnings, blocked, block_reason`) e usar esses valores diretamente nos `INSERT`s de `anthropometrics` e `nutrition_goals`. Preserva integralmente o que as migrations `0038` (username/avatar_url + `phone_e164` movido pra `profiles_private`) e `0024` (effective_from pelo dia nutricional do usuário, não `CURRENT_DATE` do servidor) já corrigiam — descoberto e validado via smoke test SQL end-to-end (transação com rollback) durante a implementação, não só nos testes unitários do módulo TS.
