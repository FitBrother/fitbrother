...
export function PostCard({ post }: { post: Post }) {
  ...
  return (
    // `overflow-hidden` e sem `p-4` no card: o padding desceu para cada bloco
    // porque a foto agora vai de ponta a ponta, e é o raio daqui que recorta os
    // cantos dela — mesma estrutura do `MealCard`.
    <View style={shadows.card} className="overflow-hidden rounded-[26px] bg-white">
      <View className="px-4 pt-4">
        {/* Cabeçalho leva ao perfil de quem postou. Um feed onde não dá pra
            tocar no nome de alguém é um mural, não uma rede — e descobrir gente
            pelos posts é o caminho mais natural para seguir alguém novo. */}
        <Pressable
      ...
      {/* Foto de ponta a ponta, em 4:5. Sem margem lateral e sem raio próprio:
          quem arredonda é o card. Antes era `h-64` fixo — um deitado ~4:3 com
          o recorte decidido pelo navegador, prato cortado sem ninguém ter
          escolhido onde. */}
      {hasImageArea ? (
        <View
          style={{ aspectRatio: FEED_PHOTO_ASPECT }}
        ...
        {achievement ? (
          ...
          <View className="mt-4 flex-row items-center gap-3 rounded-2xl bg-primary-50 p-4">
            ...
            <View className="flex-1">
              <Text className="text-xs font-sans-semibold uppercase text-primary-700">
                Conquista
              </Text>
